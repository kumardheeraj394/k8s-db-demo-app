...
+-----------------------------------------------------------+
9. Static Pod
Static Pods: Managed directly by the kubelet on a node, without API Server involvement.

Used for critical pods like kube-proxy or DNS components.

Defined via static manifest files on the node.
